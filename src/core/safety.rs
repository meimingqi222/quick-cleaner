...
/// JetBrains 在 `/Applications/JetBrains/` 下做**版本化安装**：每个版本一个
/// 目录（`IntelliJIDEA2026.2`、`PyCharm2024.3`……），旧版本不会随更新卸载，
/// 对用户纯属冗余。这里是收录进发现列表的产品表；表里存展示名，匹配时
/// 与目录名一起折叠（去空格/连字符、转小写）。
#[cfg(target_os = "macos")]
...
///
/// 放在这里而不是 categories，是因为**发现列表和清理放行必须用同一套
/// 解析**——两处各写一份，哪天改了产品表另一处必然漂移。解析不出的
/// （`Daemon`、`Toolbox`、任意其它名字）返回 `None`：分不清新旧的一律
/// 不列入、更不放行。
#[cfg(target_os = "macos")]
pub(crate) fn parse_jetbrains_versioned_name(
    name: &str,
...
/// 「旧版 IDE 安装」类目的清理放行判断——`/Applications` 受保护前缀下
/// **唯一**的窄口子（与 [`is_elevated_residual_target`] 同一范式）。
///
/// 全部命中才放行：
/// 1. 路径恰好是 `/Applications/JetBrains/<name>` 这一层，更深处不放行
///    （防止清到 bundle 内部任意子目录）；
/// 2. `<name>` 能解析为「产品 + 年.次版本」；
/// 3. 本体是真实目录，符号链接不放行（提权上下文里一条链接就能把删除
///    引到任意路径）；
/// 4. **清理瞬间重算**：同产品兄弟目录中仍存在版本号严格更高的一个。
///    扫描后用户若手动删掉了新版本，剩下的旧版本就不再是冗余——不能
///    按过期列表把它清掉。
///
/// 只有 `clean_targets` 的 OldIdeInstall 分支允许调用；磁盘透镜
/// （`clean_arbitrary`）与其它任何路径都不许用，`/Applications` 的通用
/// 保护对它们原样生效。
#[cfg(target_os = "macos")]
pub fn is_old_ide_install_target(path: &Path) -> bool {
    let lower = norm(path);
    let Some(rest) = lower
        .strip_prefix("\\applications\\jetbrains\\")
        .filter(|r| !r.is_empty() && !r.contains('\\'))
    else {
        return false;
    };
    old_ide_version_is_redundant(path, rest)
}

/// 形状检查之后的现场重验：本体真实存在、且确有更高版本的同产品兄弟。
/// 独立成函数是为了可以用临时目录做单测（外层路径是写死的
/// `/Applications/JetBrains`，测试里造不出来）。
#[cfg(target_os = "macos")]
fn old_ide_version_is_redundant(path: &Path, folded_name: &str) -> bool {
    let Some((family, version)) = parse_jetbrains_versioned_name(folded_name) else {
        return false;
    };
    ...
    entries.flatten().any(|e| {
        ...
        parse_jetbrains_versioned_name(&name)
            .is_some_and(|(f, v)| f == family && v > version)
    })
}
...
mod tests {
    ...
    /// JetBrains 目录名解析：产品 + 年.次版本。大小写、空格、连字符都要
    /// 容忍（真实目录名是 `IntelliJIDEA2026.2` 这种无空格形态）。
    #[cfg(target_os = "macos")]
    #[test]
    fn parses_jetbrains_versioned_names() {
        ...
        assert_eq!(
            parse_jetbrains_versioned_name("PyCharm2024.3"),
            Some(("PyCharm", (2024, 3)))
        );
        // 展示名形态（带空格）与三段版本号都要认
        assert_eq!(
        ...
        // 分不清新旧的一律 None：无版本、非数字版本、表外产品
        assert_eq!(parse_jetbrains_versioned_name("Daemon"), None);
        assert_eq!(parse_jetbrains_versioned_name("Toolbox"), None);
        assert_eq!(parse_jetbrains_versioned_name("Safari"), None);
        assert_eq!(parse_jetbrains_versioned_name("PyCharmEdu"), None);
        assert_eq!(parse_jetbrains_versioned_name("PyCharmLatest"), None);
    }

    /// 放行谓词的纯路径形状部分。这里只测「形状就该被拒」的输入——
    /// 形状通过的会去碰真实文件系统，而 /Applications/JetBrains 在
    /// 开发机上真实存在，结果不可复现，那部分逻辑由
    /// `old_ide_redundancy_is_recomputed` 用临时目录覆盖。
    #[cfg(target_os = "macos")]
    #[test]
    fn old_ide_gate_rejects_wrong_shapes() {
        // /Applications 下其它任何东西都不放行
        assert!(!is_old_ide_install_target(Path::new("/Applications/Safari.app")));
        // 父目录不是 /Applications/JetBrains
        assert!(!is_old_ide_install_target(Path::new(
            "/Applications/Other/PyCharm2024.3"
        )));
        // 只接受恰好一层：bundle 内部更深的路径不放行
        assert!(!is_old_ide_install_target(Path::new(
            "/Applications/JetBrains/IntelliJIDEA2025.2/Contents"
        )));
        // 解析不出版本的名字（Daemon）在形状这关就挡下
        assert!(!is_old_ide_install_target(Path::new(
            "/Applications/JetBrains/Daemon"
        )));
        // 裸的 /Applications/JetBrains 本身不放行
        assert!(!is_old_ide_install_target(Path::new("/Applications/JetBrains")));
    }

    /// 冗余判定必须在清理瞬间重算：只有「同产品确有更高版本兄弟」才算冗余。
    #[cfg(target_os = "macos")]
    #[test]
    fn old_ide_redundancy_is_recomputed() {
        let base = std::env::temp_dir().join("qc_old_ide_gate");
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(base.join("IntelliJIDEA2026.2")).unwrap();
        std::fs::create_dir_all(base.join("IntelliJIDEA2025.2")).unwrap();
        std::fs::create_dir_all(base.join("PyCharm2024.3")).unwrap();

        let old = base.join("IntelliJIDEA2025.2");
        assert!(old_ide_version_is_redundant(&old, "intellijidea2025.2"));
        // 最新版本自身不是冗余
        assert!(!old_ide_version_is_redundant(
            &base.join("IntelliJIDEA2026.2"),
            "intellijidea2026.2"
        ));
        // 没有更高版本兄弟的产品不动
        assert!(!old_ide_version_is_redundant(
            &base.join("PyCharm2024.3"),
            "pycharm2024.3"
        ));

        // 扫描后新版本被手动删掉：旧版本此刻不再是冗余，必须放它一马
        std::fs::remove_dir_all(base.join("IntelliJIDEA2026.2")).unwrap();
        assert!(!old_ide_version_is_redundant(&old, "intellijidea2025.2"));

        // 符号链接不放行：一条链接就能把删除引到任意路径。链接名刻意
        // 取可解析的 `PyCharm2023.1`，确保挡下它的是符号链接检查本身，
        // 而不是名字解析失败。
        #[cfg(unix)]
        std::os::unix::fs::symlink(
            base.join("IntelliJIDEA2025.2"),
            base.join("PyCharm2023.1"),
        )
        .ok();
        #[cfg(unix)]
        assert!(!old_ide_version_is_redundant(
