//! macOS 清理目标占用检测：正在运行的应用 + 被进程打开的文件。
//!
...
pub fn detect_inuse(targets: &[PathBuf]) -> HashMap<PathBuf, Busy> {
...
/// 判断，见 [`spot_check_inuse`] 里的说明。带超时的执行本身在 `core::proc`；
...
pub fn spot_check_inuse(paths: &[PathBuf]) -> HashMap<PathBuf, SpotCheck> {
    ...
    // 成空闲"的数据丢失风险，跟 detect_inuse 里 ps 失败必须 fail closed
    // 不是同一类问题。
    let procs = ps_snapshot().unwrap_or_default();
    let excluded = self_and_descendant_pids(std::process::id(), &procs);

    // 同 detect_inuse：lsof 报内核解析后的真实路径，回查前先各自
    // canonicalize 一份。
    let mut want: HashMap<PathBuf, PathBuf> = HashMap::new();
    for p in &existing {
...
mod tests {
    ...
    fn open_file_marks_target_via_ancestors() {
        ...
        // 模拟 detect_inuse 的 want 映射与父链回溯。lsof 报告的是内核
    ...
    fn mark_all_unknown_sets_unknown_without_clobbering_existing_fields() {
        // 证明 ps 失败会让 detect_inuse 整轮变成 unknown：detect_inuse
    ...
    fn spot_check_clears_nonexistent_paths_without_calling_lsof() {
        ...
        let result = spot_check_inuse(std::slice::from_ref(&ghost));
