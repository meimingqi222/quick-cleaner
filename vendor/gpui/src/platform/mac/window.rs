...
extern "C" fn window_did_change_key_status(this: &Object, selector: Sel, _: id) {
    ...
    if selector == sel!(windowDidBecomeKey:) && !is_active {
        // PATCH(quick-cleaner): drop the state lock before resignKeyWindow.
        // resignKeyWindow synchronously posts NSWindowDidResignKeyNotification,
        // which reenters this callback on the same thread; parking_lot Mutex
        // is not reentrant so holding the lock here deadlocks the main thread
        // (observed on macOS 27 during window state restoration).
        // Matches upstream zed main (gpui_macos/src/window.rs).
