...
pub fn tr_status_residual_cleaned_manual(
    ...
    match lang {
        Language::Zh => format!(
            "已清除「{name}」的 {count} 项残留，释放 {size}（{manual} 项需重启后由系统清除，或在系统设置中处理）"
        ),
        Language::En => format!(
            "Removed {count} leftovers of \"{name}\", freed {size} ({manual} need a reboot or manual cleanup in System Settings)"
        ),
    }
}
