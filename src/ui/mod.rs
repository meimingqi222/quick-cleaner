...
impl Root {
    ...
    fn refresh_apps_view(&mut self) {
        // 输入法组词期间不重排：失效键含原始输入文本，拼音串会让缓存每帧
        // 失配、拿半截拼音去过滤——列表先清空、选字确认后又跳回来。组词
        // 结束（marked 归 None）时文本已是最终值，下一帧自然失配重算。
        // 文件搜索没有这个问题：它的结果只由 `search_input_changed` 的
        // 防抖任务更新，组词期间在 trait 层就已经被拦下了。
        if self.apps.input.marked.is_some() {
            return;
        }
